...
class SpringSpec: QuickSpec {
  ...
  override func spec() {
    ...
    it("is created at rest") {
      expect(spring.isAtRest).to(beTrue())
      expect(spring.currentValue).to(equal(0))
      expect(spring.endValue).to(equal(0))
      expect(spring.velocity).to(equal(0))
    }
    
    it("can have listeners") {
      let removeToken = spring.addListener()
      expect(spring.getListenerCount()).to(equal(1))
      spring.removeListener(removeToken)
      expect(spring.getListenerCount()).to(equal(0))
      spring.addListener()
      spring.addListener()
      spring.addListener()
      spring.addListener()
      expect(spring.getListenerCount()).to(equal(4))
    ...
    it("performs the expected numerical integration") {
      ...
      expect(actualValues).to(beCloseTo(expectedValues))
      expect(expectedVelocities).to(beCloseTo(expectedVelocities))
    }
    ...
    it("should not oscillate if overshoot clamping is enabled") {
      ...
      expect(didOscillate).to(beFalse())
    }
    ...
    it("should be at rest after calling setCurrentValue") {
      ...
      expect(spring.isAtRest).to(beTrue())
      expect(spring.currentValue).to(equal(-1.0))
      expect(spring.endValue).to(equal(-1.0))
    }
    ...
    it("should not be at rest if the skipSetAtRest parameter is passed to setCurrentValue while moving") {
      ...
      expect(spring.isAtRest).to(beFalse())
      expect(spring.currentValue).to(equal(-1.0))
      expect(spring.endValue).to(equal(1.0))
    }
